...
oraz E2E (Playwright) dla każdej ścieżki P1. Dla US5 (R-34): jednostkowe adapterów metryk
(`SimpleMeterRegistry`), asercje liczników po commicie i rollbacku w testach `*Service`,
`MetrykiEndpointIT`, `promtool test rules` i `scripts/check-dashboards.mjs`. Testy piszemy PRZED
implementacją i muszą najpierw nie przechodzić.

**Organization**: Zadania pogrupowane wg historii użytkownika (US1–US4 — P1, US5 — P2), tak by
...
- **[Story]**: historia użytkownika, do której należy zadanie (US1, US2, US3, US4, US5)
...
- Obserwowalność: `observability/prometheus/...`, `observability/grafana/...`, `scripts/check-dashboards.mjs`
...
- [ ] T020 [P] Zaimplementuj `StripeProperties` jako `@ConfigurationProperties("shop.stripe") record StripeProperties(String secretKey, String webhookSecret, Duration connectTimeout, Duration readTimeout, @DefaultValue("https://api.stripe.com") URI apiBase)` (R-20) — **bez** pola `maxNetworkRetries`: ponawianie to polityka adaptera, a SDK zawsze dostaje `0` (R-13, T102). Walidacja w konstruktorze kompaktowym rzuca `IllegalArgumentException` z komunikatami BEZ wartości: brak/pusty `secretKey` → „Brak wymaganej konfiguracji: shop.stripe.secret-key”; `secretKey` niepasujący do `^sk_test_.+` (np. `sk_live_…`) → „shop.stripe.secret-key musi być kluczem testowym (sk_test_…)”; analogicznie `webhookSecret` (`^whsec_.+`, „Brak wymaganej konfiguracji: shop.stripe.webhook-secret”); `connectTimeout`/`readTimeout` null lub ≤ 0 → błąd z nazwą właściwości. NIE używaj `@Validated` + `@Pattern`: błąd Bean Validation zawiera `rejected value [sk_live_…]`, co łamie T013. Nadpisz `toString()` maskując oba sekrety (`sk_test_***`, `whsec_***`) — domyślny `toString()` rekordu wypisałby je do logu. Dodaj `StripePropertiesFailureAnalyzer` (`AbstractFailureAnalyzer<BindException>` dla celu `StripeProperties`, `@Order(Ordered.HIGHEST_PRECEDENCE)`, zarejestrowany w `backend/src/main/resources/META-INF/spring.factories`), który raportuje tylko nazwę właściwości i komunikat przyczyny — standardowy `BindFailureAnalyzer` wypisuje `Value: "…"`. Pliki: `backend/src/main/java/com/project/custom/platnosc/infrastructure/stripe/StripeProperties.java`, `backend/src/main/java/com/project/custom/platnosc/infrastructure/stripe/StripePropertiesFailureAnalyzer.java`
...
- [ ] T086 [P] [US4] Test awarii adaptera na WireMock (polityka ponowień z T102): timeout odczytu, zerwane połączenie, `5xx`, `429` → `BramkaNiedostepna` po dokładnie 2 ponowieniach — WireMock odnotowuje dokładnie 3 żądania, wszystkie z tym samym `Idempotency-Key: checkout-{platnoscId}` (dowód, że SDK z `maxNetworkRetries=0` nie dokłada własnych prób); opóźnienia 0,5 s i 1 s zarejestrowane przez testowy `Sleeper` bez realnego czekania; `503` → `200` za drugim razem → `SesjaPlatnosci` po 1 ponowieniu; `400`/`401` → dokładnie 1 żądanie, bez ponowień, błąd konfiguracji bez klucza w komunikacie/logu; odpowiedź `expire` „session is not open” ze `status=complete` → `JUZ_OPLACONA` — w `backend/src/test/java/com/project/custom/platnosc/infrastructure/stripe/StripeBramkaPlatnosciAwarieTest.java`
...
- [ ] T102 [US4] Zaimplementuj package-private adapter `StripeBramkaPlatnosci` implementujący port `BramkaPlatnosci` wraz z konfiguracją klienta i polityką ponowień (R-13, `contracts/stripe-webhook.md` §1):
  - **Klient** (`StripeClientConfig`): `StripeClient.builder()` z `StripeProperties` — `setApiKey(secretKey)`, `setApiBase(apiBase)`, `setConnectTimeout(connectTimeout)`, `setReadTimeout(readTimeout)` i **`setMaxNetworkRetries(0)`** — SDK nie ponawia, żeby każdą próbę dało się policzyć w `shop.stripe.ponowienia` (US5).
  - **Ponowienia w adapterze** (`PolitykaPonowienStripe`, package-private, stałe z kontraktu): maks. 2 ponowienia (3 próby), opóźnienia 0,5 s i 1 s przez wstrzykiwany `Sleeper` (domyślnie `Thread.sleep`, w testach atrapa). Ponawiane są tylko: `APIConnectionException` (w tym timeout — przyczyna `SocketTimeoutException`/`HttpTimeoutException`), `StripeException` ze `statusCode ≥ 500` i `RateLimitException` (`429`). Każda próba idzie z tym samym `RequestOptions.setIdempotencyKey("checkout-{platnoscId}")`. Po wyczerpaniu prób → `BramkaNiedostepna`.
  - **Bez ponowień**: `InvalidRequestException`/`AuthenticationException`/`PermissionException` → błąd konfiguracji (`500`), log bez klucza i bez danych klienta.
  - **Klasyfikacja prób**: enumy `OperacjaStripe {UTWORZ_SESJE, WYGAS_SESJE}` i `WynikWywolania {SUKCES, BLAD, TIMEOUT}` (data-model.md); przy ponowieniu log `WARN` z operacją, numerem próby i wynikiem. Metryki dopina T141 (US5), bez zmiany tej logiki.
  - **`utworzSesje`**: parametry wg kontraktu (`mode=payment`, `payment_method_types[]=card`, `currency=pln`, `unit_amount` w groszach, `quantity`, nazwa pozycji, `customer_email`, `client_reference_id`, `metadata[zamowienieId|platnoscId|numer]`, `expires_at` = teraz + 30 min z wstrzykiwanego `Clock`, `success_url`/`cancel_url` z `AppProperties.baseUrl`). Przed wywołaniem asercja `Σ unit_amount × quantity == suma`.
  - **`wygas`**: `sessions.expire` z tą samą polityką ponowień; błąd „session is not open” → `sessions.retrieve`: `status=complete` → `JUZ_OPLACONA`, `status=expired` → `WYGASZONA`.
  - Pliki w `backend/src/main/java/com/project/custom/platnosc/infrastructure/stripe/`: `StripeBramkaPlatnosci.java`, `StripeClientConfig.java`, `PolitykaPonowienStripe.java`, `Sleeper.java`, `OperacjaStripe.java`, `WynikWywolania.java`.
...
## Phase 7: User Story 5 — Monitorowanie działania sklepu (Priority: P2)

**Goal**: Metryki Prometheus na wewnętrznym porcie `8081` (HTTP, JVM, HikariCP, Flyway, metryki
biznesowe ścieżki zakupowej, wywołania Stripe, webhooki, Outbox), zapisywane dopiero po commicie;
`docker compose up -d` podnosi Prometheusa z 9 regułami alertów i Grafanę z 4 dashboardami
z provisioningu; identyfikator korelacji `X-Request-Id` w logach (FR-025–FR-034, SC-009–SC-012).
Katalog nazw i etykiet — wyłącznie [contracts/metrics.md](contracts/metrics.md).

**Independent Test**: `docker compose up -d` + backend → Grafana ma folder „Sklep” z 4 dashboardami
i danymi w ≤ 2 min; `:8080/actuator/prometheus` → `404`, `:8081` → metryki `shop_*` z
`application="shop"` bez PII; zakup `4242…` → +1 utworzone/`OPLACONE`, wartość sprzedaży, +1
`udana` w ≤ 1 min; karta `4000…0002` → `odrzucona`; sfałszowany podpis → `odrzucony_podpis`
i alert `SfalszowanePotwierdzeniePlatnosci` „firing”; zatrzymany backend → `SklepNiedostepny`;
każda odpowiedź `/api/**` ma `X-Request-Id` obecny w logu (quickstart 5.1–5.12); w CI `promtool`
pokazuje `firing` i `resolved` każdej reguły.

**Zależności**: wymaga US2–US4 (metryki wpinane w `KoszykService`, `ZlozZamowienieService`,
`ObslugaPlatnosciListener`, `ObslugaWebhookaService`, `StripeBramkaPlatnosci`). Konfiguracja
Actuatora, zależność Micrometera i reguła ArchUnit dla `io.micrometer` są już w T001, T012, T015.
Pliki `observability/**` (T132, T133, T143–T150) zależą tylko od kontraktu i mogą powstawać od
razu po Phase 2.

### Testy dla User Story 5 ⚠️

- [ ] T120 [P] [US5] Test jednostkowy `PoCommicie` bez kontekstu Springa (ręczne `TransactionSynchronizationManager.initSynchronization()`): bez aktywnej synchronizacji akcja wykonuje się natychmiast; przy synchronizacji — dopiero w `afterCommit`; przy `afterCompletion(STATUS_ROLLED_BACK)` — wcale; wyjątek w akcji jest logowany i nie przerywa commitu — w `backend/src/test/java/com/project/custom/shared/infrastructure/metrics/PoCommicieTest.java`
- [ ] T121 [P] [US5] Test `KorelacjaFilter` (R-32): brak nagłówka → odpowiedź z `X-Request-Id` będącym UUID; poprawny `X-Request-Id: abc-12345` → ta sama wartość w odpowiedzi; niepoprawny (7 znaków, 65 znaków, `<script>`, CR/LF) → nowy UUID; w trakcie obsługi `MDC.get("requestId")` równe nagłówkowi, po żądaniu MDC wyczyszczone (także po wyjątku); filtr działa przed `GoscIdFilter` — w `backend/src/test/java/com/project/custom/shared/api/KorelacjaFilterTest.java`
- [ ] T122 [P] [US5] Test jednostkowy adaptera `MicrometerMetrykiKoszyka` na `SimpleMeterRegistry` (bez Springa, `PoCommicie` bez transakcji → natychmiast): `dodanoDoKoszyka()` → `shop.koszyk.dodania` +1; licznik zarejestrowany z `0` po utworzeniu adaptera — w `backend/src/test/java/com/project/custom/koszyk/infrastructure/metrics/MicrometerMetrykiKoszykaTest.java`
- [ ] T123 [P] [US5] Test jednostkowy adaptera `MicrometerMetrykiZamowien` (`SimpleMeterRegistry`): `zamowienieUtworzone()` → `shop.zamowienia.utworzone` +1; `rozbieznoscPodsumowania({CENA, SKLAD})` → +1 dla `rodzaj=CENA` i `rodzaj=SKLAD`, `DOSTEPNOSC` bez zmian; `zamowienieZakonczone(OPLACONE, 123,45 zł, 90 s)` → `shop.zamowienia.zakonczone{status=OPLACONE}` +1, `shop.zamowienia.oplacone.wartosc` +123.45 (base unit `pln`), timer `shop.zamowienie.czas.do.oplacenia` z kubełkami 30 s, 1 min, 2 min, 5 min, 10 min, 30 min; `PLATNOSC_NIEUDANA`/`WYMAGA_WYJASNIENIA` → tylko licznik statusu (bez wartości i czasu); wszystkie kombinacje `rodzaj` i `status` zarejestrowane z `0` przy starcie — w `backend/src/test/java/com/project/custom/zamowienie/infrastructure/metrics/MicrometerMetrykiZamowienTest.java`
- [ ] T124 [P] [US5] Test jednostkowy adaptera `MicrometerMetrykiPlatnosci` (`SimpleMeterRegistry`): `webhook(WynikWebhooka)` → `shop.platnosc.webhook{wynik}` z wartościami małymi literami `przetworzone|zduplikowane|odrzucony_podpis|odrzucony_livemode|zignorowane`; `platnosc(WynikPlatnosci)` → `shop.platnosci{wynik=udana|odrzucona|anulowana}`; `opoznieniePotwierdzenia(12 s)` → timer `shop.platnosc.opoznienie.potwierdzenia` z kubełkami 1, 5, 10, 30, 60, 120 s; wszystkie kombinacje zarejestrowane z `0` — w `backend/src/test/java/com/project/custom/platnosc/infrastructure/metrics/MicrometerMetrykiPlatnosciTest.java`
- [ ] T125 [P] [US5] Test metryk adaptera Stripe (WireMock + `SimpleMeterRegistry`, testowy `Sleeper`): sukces → 1 próba w `shop.stripe.wywolania{operacja=utworz_sesje,wynik=sukces}`, `shop.stripe.ponowienia` bez zmian; `503` ×3 → 3 próby `wynik=blad` i `shop.stripe.ponowienia{operacja=utworz_sesje}` = 2; timeout odczytu → `wynik=timeout`; `400` → 1 próba `blad`, 0 ponowień; `expire` → `operacja=wygas_sesje`; timer z kubełkami 100 ms, 300 ms, 1 s, 3 s, 10 s; wszystkie kombinacje `operacja × wynik` zarejestrowane z `0` — w `backend/src/test/java/com/project/custom/platnosc/infrastructure/stripe/StripeBramkaPlatnosciMetrykiTest.java`
- [ ] T126 [P] [US5] Utwórz pomocnika asercji metryk `MetrykiAssert` (odczyt licznika/timera z `MeterRegistry` po nazwie i etykietach, `zmiana(() -> akcja)` zwracająca przyrost, brak metryki = `0`) w `backend/src/test/java/com/project/custom/support/MetrykiAssert.java`
- [ ] T127 [US5] Rozszerz `KoszykServiceIT` o metryki (używa `MetrykiAssert` z T126): udane `POST /api/koszyk/pozycje` → `shop.koszyk.dodania` +1 po commicie; `409 ILOSC_PRZEKRACZA_LIMIT`, `409 PRODUKT_NIEDOSTEPNY`, `400` → bez zmian; wymuszony rollback po zapisie (np. `KONFLIKT_WSPOLBIEZNOSCI`) → bez zmian (SC-010) — w `backend/src/test/java/com/project/custom/koszyk/application/KoszykServiceIT.java`
- [ ] T128 [US5] Rozszerz `ZlozZamowienieServiceIT` o metryki: `201` → `shop.zamowienia.utworzone` +1; `409 PODSUMOWANIE_NIEAKTUALNE` ze zmienioną ceną → `rozbieznosci{rodzaj=CENA}` +1, z `stan = 0` → `DOSTEPNOSC`, z innym zestawem produktów → `SKLAD` (jedna inkrementacja na wykryty rodzaj), `utworzone` bez zmian; `503 PLATNOSC_NIEDOSTEPNA` → `utworzone` +1 (TX1 zatwierdzona) i `zakonczone{status=PLATNOSC_NIEUDANA}` +1; `409 KOSZYK_NIE_DO_ZAMOWIENIA`/`400` → bez zmian — w `backend/src/test/java/com/project/custom/zamowienie/application/ZlozZamowienieServiceIT.java`
- [ ] T129 [US5] Rozszerz `ObslugaWebhookaServiceIT` o metryki wg tabeli z `contracts/stripe-webhook.md` §2 „Metryki”: `completed/paid` → `webhook{przetworzone}` +1, `platnosci{udana}` +1, `zakonczone{OPLACONE}` +1, `oplacone_wartosc_pln` + suma/100, `czas.do.oplacenia` i `opoznienie.potwierdzenia` (= teraz − `event.created`) zarejestrowane; ten sam `event.id` 2× → `zduplikowane` +1, reszta bez zmian; błędny podpis → `odrzucony_podpis` +1; `livemode=true` → `odrzucony_livemode` +1; `payment_intent.payment_failed` → `przetworzone` +1 i `platnosci{odrzucona}` +1 BEZ zmiany `Platnosc` i `Zamowienie` (R-28); `async_payment_failed` → `odrzucona` + `zakonczone{PLATNOSC_NIEUDANA}`; `expired` → `anulowana`; `completed` z `payment_status != paid`, nieznana sesja, inny typ → `zignorowane`; niewystarczający stan → `zakonczone{WYMAGA_WYJASNIENIA}`; spóźnione potwierdzenie po niepowodzeniu → `WYMAGA_WYJASNIENIA` +1, a `PLATNOSC_NIEUDANA` bez zmian; wymuszony rollback transakcji webhooka (błąd w listenerze → `500`) → `przetworzone`, `udana`, `zakonczone` bez zmian — w `backend/src/test/java/com/project/custom/platnosc/application/ObslugaWebhookaServiceIT.java`
- [ ] T130 [P] [US5] Test integracyjny gauge'y: `shop.outbox.oczekujace` i `shop.outbox.najstarsze` — pusty Outbox → `0`/`0`; 2 wiersze z `wyslano IS NULL` (najstarszy sprzed 400 s) i 1 wysłany → `2`/`≈ 400`; wynik buforowany 15 s (wstrzykiwany `Clock`: druga zmiana w bazie niewidoczna przed upływem 15 s); `shop.flyway.migracje{stan=success}` = liczba migracji `V1…V5`, `failed`/`pending` = `0` — w `backend/src/test/java/com/project/custom/shared/infrastructure/metrics/GaugeMetrykiIT.java`
- [ ] T131 [P] [US5] Test `MetrykiEndpointIT` (R-33, SC-012; `@SpringBootTest(webEnvironment = RANDOM_PORT)`, port zarządzania z `@LocalManagementPort`): wykonuje pełną ścieżkę (dodanie do koszyka → `POST /api/zamowienia` na `stripe-mock` → webhook udany, zduplikowany i sfałszowany), pobiera `/actuator/prometheus` z portu zarządzania i sprawdza: (1) brak wzorców e-maila, imienia i nazwiska, ulicy, `ZAM-[0-9A-Z]{10}`, `cs_test_`, `pi_`, `evt_`, `sk_`, `whsec_`, wartości `shop_guest` i `X-Request-Id` użytych w teście; (2) każda seria `shop_*` ma `application="shop"`; (3) zbiór nazw `shop_*` (bez sufiksów `_bucket|_count|_sum|_max`) równy liście z `contracts/metrics.md` §2; (4) `GET :{port API}/actuator/prometheus` → `404`; (5) `/actuator/health/readiness` na porcie zarządzania → `UP` bez szczegółów — w `backend/src/test/java/com/project/custom/MetrykiEndpointIT.java`
- [ ] T132 [P] [US5] Napisz testy reguł `promtool` z przypadkiem `firing` i `resolved` dla każdej z 9 reguł z `contracts/metrics.md` §4 (dane syntetyczne `input_series`, `eval_time` po upływie `for`, oczekiwane etykiety `application`, `waga`, dla `OutboxZalegly` także `wymaga="realizacja"`) w `observability/prometheus/tests/shop.test.yml`
- [ ] T133 [P] [US5] Utwórz walidator `scripts/check-dashboards.mjs` (Node, bez zależności): parsuje tabelę §2 z `specs/001-shop-browse-cart-checkout/contracts/metrics.md` jako listę dozwolonych metryk `shop_*`; dla każdego `observability/grafana/dashboards/*.json` sprawdza poprawny JSON, `datasource.uid = "prometheus"` we wszystkich panelach i celach, że każda metryka `shop_*` z wyrażeń jest na liście, a liczniki (`_total`, `_count`) występują wyłącznie w `rate(`/`increase(`; to samo dla wyrażeń w `observability/prometheus/rules/*.yml`; kod wyjścia `1` z listą błędów

### Implementacja User Story 5 — wspólne jądro

- [ ] T134 [P] [US5] Zaimplementuj `public final class PoCommicie` (`wykonaj(Runnable)`: przy aktywnej synchronizacji transakcji rejestruje `TransactionSynchronization.afterCommit`, bez niej wykonuje natychmiast; wyjątek z akcji łapany i logowany na `WARN`, nigdy nie przerywa ścieżki zakupowej) w `backend/src/main/java/com/project/custom/shared/infrastructure/metrics/PoCommicie.java`
- [ ] T135 [P] [US5] Zaimplementuj `MetrykiConfig` (`@Configuration`; bean `MeterFilter.maximumAllowableTags("http.server.requests", "uri", 50, MeterFilter.deny())` jako bezpiecznik kardynalności, R-26) w `backend/src/main/java/com/project/custom/shared/infrastructure/metrics/MetrykiConfig.java`
- [ ] T136 [P] [US5] Zaimplementuj `KorelacjaFilter` (`OncePerRequestFilter` dla `/api/**`: nagłówek `X-Request-Id` przyjmowany tylko przy dopasowaniu `^[A-Za-z0-9-]{8,64}$`, inaczej `UUID.randomUUID()`; `MDC.put("requestId", …)`, nagłówek w odpowiedzi, `MDC.remove` w `finally`; wartość nigdy nie trafia do metryk) w `backend/src/main/java/com/project/custom/shared/api/KorelacjaFilter.java` i zarejestruj go przed `GoscIdFilter` (`FilterRegistrationBean` z niższym `order`) w `backend/src/main/java/com/project/custom/shared/api/WebConfig.java`

### Implementacja User Story 5 — metryki biznesowe w BC

- [ ] T137 [US5] Dodaj metryki koszyka: port `public interface MetrykiKoszyka { void dodanoDoKoszyka(); }` w `backend/src/main/java/com/project/custom/koszyk/application/MetrykiKoszyka.java`; package-private adapter `MicrometerMetrykiKoszyka` (licznik `shop.koszyk.dodania` rejestrowany w konstruktorze, inkrementacja przez `PoCommicie`) w `backend/src/main/java/com/project/custom/koszyk/infrastructure/metrics/MicrometerMetrykiKoszyka.java`; wywołanie portu w `KoszykService.dodaj` po udanym dodaniu (wewnątrz transakcji — adapter odkłada zapis do commitu) w `backend/src/main/java/com/project/custom/koszyk/application/KoszykService.java`
- [ ] T138 [US5] Dodaj metryki zamówień: enum `RodzajRozbieznosci {CENA, DOSTEPNOSC, SKLAD}` i port `public interface MetrykiZamowien` (`zamowienieUtworzone()`, `rozbieznoscPodsumowania(Set<RodzajRozbieznosci>)`, `zamowienieZakonczone(StatusZamowienia docelowy, Pieniadze suma, Duration czasDoOplacenia)`) w `backend/src/main/java/com/project/custom/zamowienie/application/`; package-private adapter `MicrometerMetrykiZamowien` (nazwy, etykiety i kubełki z `contracts/metrics.md` §2; wartość PLN = `grosze / 100.0` tylko do prezentacji; wszystkie kombinacje z `0` przy starcie; zapis przez `PoCommicie`) w `backend/src/main/java/com/project/custom/zamowienie/infrastructure/metrics/MicrometerMetrykiZamowien.java`; wpięcie: `ZlozZamowienieService` — `zamowienieUtworzone()` w TX1, przy `PodsumowanieNieaktualne` wylicz rodzaje (inna cena jednostkowa → `CENA`; pozycja niedostępna lub ilość > stan → `DOSTEPNOSC`; inny zbiór produktów lub ilości → `SKLAD`) i wywołaj `rozbieznoscPodsumowania` natychmiast (bez transakcji), przy `BramkaNiedostepna` `zamowienieZakonczone(PLATNOSC_NIEUDANA, …)` w TX oznaczenia niepowodzenia; `ObslugaPlatnosciListener` — `zamowienieZakonczone` tylko przy faktycznym przejściu statusu (`OPLACONE → OPLACONE` nic nie liczy), `czasDoOplacenia = oplacono − utworzono` — w `backend/src/main/java/com/project/custom/zamowienie/application/ZlozZamowienieService.java` i `backend/src/main/java/com/project/custom/zamowienie/application/ObslugaPlatnosciListener.java`
- [ ] T139 [US5] Rozszerz `PotwierdzenieOperatora` o `Instant utworzonoUOperatora` (z `event.created`) w `backend/src/main/java/com/project/custom/platnosc/domain/PotwierdzenieOperatora.java` oraz `StripeWebhookVerifier` o mapowanie tego pola i typu `payment_intent.payment_failed` (bez `sesjaId`; tylko do metryki, R-28) w `backend/src/main/java/com/project/custom/platnosc/infrastructure/stripe/StripeWebhookVerifier.java`
- [ ] T140 [US5] Dodaj metryki płatności: enumy `WynikWebhooka {PRZETWORZONE, ZDUPLIKOWANE, ODRZUCONY_PODPIS, ODRZUCONY_LIVEMODE, ZIGNOROWANE}`, `WynikPlatnosci {UDANA, ODRZUCONA, ANULOWANA}` i port `public interface MetrykiPlatnosci` (`webhook(WynikWebhooka)`, `platnosc(WynikPlatnosci)`, `opoznieniePotwierdzenia(Duration)`) w `backend/src/main/java/com/project/custom/platnosc/application/`; package-private adapter `MicrometerMetrykiPlatnosci` (etykiety małymi literami, kubełki z kontraktu, `PoCommicie`) w `backend/src/main/java/com/project/custom/platnosc/infrastructure/metrics/MicrometerMetrykiPlatnosci.java`; w `ObslugaWebhookaService` przyjmij surowe ciało i podpis (weryfikacja przez `StripeWebhookVerifier` wewnątrz serwisu, żeby każdy wynik przeszedł przez port): odrzucenie podpisu/`livemode` → `webhook(...)` natychmiast i wyjątek mapowany przez kontroler na `400`; duplikat → `ZDUPLIKOWANE`; mapowanie wyników wg R-28 (`completed`/paid i `async_payment_succeeded` → `UDANA` + `opoznieniePotwierdzenia(teraz − utworzonoUOperatora)`; `payment_intent.payment_failed` i `async_payment_failed` → `ODRZUCONA`; `expired` → `ANULOWANA`; `payment_intent.payment_failed` tylko deduplikacja + metryka, bez zmiany `Platnosc`); pozostałe → `ZIGNOROWANE`; `MDC.put("stripeEventId", …)` na czas obsługi — w `backend/src/main/java/com/project/custom/platnosc/application/ObslugaWebhookaService.java`; uprość `StripeWebhookController` do przekazania ciała i nagłówka do serwisu w `backend/src/main/java/com/project/custom/platnosc/api/StripeWebhookController.java`
- [ ] T141 [US5] Dodaj metryki wywołań do `StripeBramkaPlatnosci` (wstrzyknięty `MeterRegistry`; każda próba z pętli ponowień T102 → timer `shop.stripe.wywolania{operacja, wynik}` z kubełkami 100 ms, 300 ms, 1 s, 3 s, 10 s; każde ponowienie → `shop.stripe.ponowienia{operacja}`; etykiety z enumów `OperacjaStripe`/`WynikWywolania` małymi literami; wszystkie kombinacje z `0` przy starcie; bez `PoCommicie` — wywołania są poza transakcją) w `backend/src/main/java/com/project/custom/platnosc/infrastructure/stripe/StripeBramkaPlatnosci.java`
- [ ] T142 [P] [US5] Zaimplementuj gauge'e w `shared`: `OutboxMetryki` (`shop.outbox.oczekujace` i `shop.outbox.najstarsze` w sekundach, jedno zapytanie `SELECT COUNT(*), MIN(utworzono) FROM outbox_event WHERE wyslano IS NULL` przez `OutboxEventJpaRepository`, wynik buforowany 15 s z wstrzykiwanym `Clock`, `0` gdy brak) i `FlywayMetryki` (`shop.flyway.migracje{stan=success|failed|pending}` z `Flyway.info().all()` raz po `ApplicationReadyEvent`) w `backend/src/main/java/com/project/custom/shared/infrastructure/metrics/OutboxMetryki.java` i `backend/src/main/java/com/project/custom/shared/infrastructure/metrics/FlywayMetryki.java`

### Implementacja User Story 5 — Prometheus, Grafana, CI

- [ ] T143 [P] [US5] Dodaj do `compose.yaml` serwisy `prometheus` (`prom/prometheus` z przypiętą wersją, `--config.file=/etc/prometheus/prometheus.yml`, `--storage.tsdb.retention.time=15d`, wolumeny `./observability/prometheus:/etc/prometheus:ro` i nazwany na dane, `extra_hosts: host.docker.internal:host-gateway`, port `127.0.0.1:9090:9090`) i `grafana` (`grafana/grafana` z przypiętą wersją, `GF_SECURITY_ADMIN_PASSWORD: ${GRAFANA_ADMIN_PASSWORD:?Ustaw GRAFANA_ADMIN_PASSWORD w .env}`, `GF_USERS_ALLOW_SIGN_UP=false`, `GF_AUTH_ANONYMOUS_ENABLED=false`, wolumeny `./observability/grafana/provisioning:/etc/grafana/provisioning:ro` i `./observability/grafana/dashboards:/var/lib/grafana/dashboards:ro`, port `127.0.0.1:3000:3000`, `depends_on: prometheus`) w profilu domyślnym; w serwisie `stripe-cli` ogranicz `listen` do `--events checkout.session.completed,checkout.session.async_payment_succeeded,checkout.session.async_payment_failed,checkout.session.expired,payment_intent.payment_failed` (R-28); dopisz `GRAFANA_ADMIN_PASSWORD=` z komentarzem „wymagane, nie używaj admin/admin” w `.env.example`
- [ ] T144 [P] [US5] Utwórz `observability/prometheus/prometheus.yml` (`global.scrape_interval: 15s`, `evaluation_interval: 15s`; `rule_files: [/etc/prometheus/rules/*.yml]`; job `shop-backend` z `metrics_path: /actuator/prometheus` i celem `host.docker.internal:8081`)
- [ ] T145 [P] [US5] Utwórz 9 reguł alertów z `contracts/metrics.md` §4 dosłownie (grupa `shop`, `interval: 15s`; wyrażenia, `for` i `waga` z tabeli; etykieta `application: shop`; adnotacje `summary` po polsku i `dashboard` z linkiem do panelu; `OutboxZalegly` z etykietą `wymaga: realizacja` i adnotacją o braku joba wysyłki, R-30) w `observability/prometheus/rules/shop.yml`
- [ ] T146 [P] [US5] Utwórz provisioning Grafany: źródło danych Prometheus (`uid: prometheus`, `url: http://prometheus:9090`, `isDefault: true`, `editable: false`) w `observability/grafana/provisioning/datasources/prometheus.yaml` i provider plików (folder „Sklep”, `path: /var/lib/grafana/dashboards`, `allowUiUpdates: false`) w `observability/grafana/provisioning/dashboards/shop.yaml`
- [ ] T147 [P] [US5] Utwórz dashboard „Sklep — HTTP” (zmienna `$application` domyślnie `shop`; żądania/s per `uri`; odsetek 4xx i 5xx; p50/p95/p99 per `uri` przez `histogram_quantile`; tabela top 5 najwolniejszych `uri`; panel stanu alertów `WysokiOdsetekBledow5xx`, `WolneOdpowiedziKataloguKoszyka`) w `observability/grafana/dashboards/http.json`
- [ ] T148 [P] [US5] Utwórz dashboard „Sklep — ścieżka zakupowa” (lejek `increase` w zakresie dashboardu: `shop_koszyk_dodania_total` → `shop_zamowienia_utworzone_total` → `shop_zamowienia_zakonczone_total{status="OPLACONE"}`; zamówienia wg `status`; płatności wg `wynik` jako osobne serie `udana`/`odrzucona`/`anulowana`; wartość sprzedaży z `shop_zamowienia_oplacone_wartosc_pln_total`; rozbieżności wg `rodzaj`) w `observability/grafana/dashboards/sciezka-zakupowa.json`
- [ ] T149 [P] [US5] Utwórz dashboard „Sklep — płatności i integracje” (wywołania Stripe wg `operacja`/`wynik`; p95 czasu wywołań; ponowienia; webhooki wg `wynik` z wyróżnionym `odrzucony_podpis`; p95 opóźnienia potwierdzenia z linią progu 30 s; rozkład czasu do opłacenia; Outbox: oczekujące i najstarsze w panelu opisanym „oczekuje na funkcję realizacja”) w `observability/grafana/dashboards/platnosci-integracje.json`
- [ ] T150 [P] [US5] Utwórz dashboard „Sklep — JVM i baza” (heap/non-heap, pauzy GC, wątki, `process_cpu_usage`, `process_uptime_seconds`, HikariCP `active`/`idle`/`pending` i czas pozyskania połączenia, `shop_flyway_migracje` wg `stan`, `up{job="shop-backend"}`) w `observability/grafana/dashboards/jvm-baza.json`
- [ ] T151 [US5] Dodaj do `.github/workflows/ci.yml` job `observability`: `promtool check config` i `promtool check rules` oraz `promtool test rules observability/prometheus/tests/shop.test.yml` (obraz `prom/prometheus` z tą samą wersją co w `compose.yaml`, `--entrypoint promtool`) i `node scripts/check-dashboards.mjs`

**Checkpoint**: US5 działa — quickstart 5.1–5.12 przechodzi, `promtool test rules`,
`check-dashboards.mjs` i `MetrykiEndpointIT` zielone; US1–US4 bez regresji.

---

## Phase 8: Polish & Cross-Cutting Concerns

**Purpose**: Kontrakt, wydajność (także narzut metryk), bezpieczeństwo i walidacja całości (SC-003, SC-008, SC-012).

- [ ] T152 [P] Zaimplementuj `OpenApiContractTest` porównujący ścieżki, metody i kody odpowiedzi z `/v3/api-docs` (springdoc) z `backend/src/main/resources/openapi/shop-api.yaml` w `backend/src/test/java/com/project/custom/OpenApiContractTest.java`
- [ ] T153 [P] Test wydajności na seedzie ≥ 500 produktów (lista, wyszukiwanie z frazą, filtr+sort, koszyk z 10 pozycjami — każde zapytanie backendu ≤ 300 ms po rozgrzaniu, brak N+1 weryfikowany licznikiem zapytań Hibernate) oraz narzut metryk (R-34, SC-012): ten sam pomiar z konfiguracją jak w produkcji i z `management.metrics.enable.all=false` — różnica p95 < 5% lub < 5 ms — w `backend/src/test/java/com/project/custom/katalog/application/WydajnoscKatalogIT.java`
- [ ] T154 [P] Test braku danych osobowych i sekretów w logach (przechwycenie logów podczas złożenia zamówienia i webhooka: brak pełnego e-maila, adresu, `sk_test_`, `whsec_`; każdy wpis z żądania API zawiera `[requestId]`) w `backend/src/test/java/com/project/custom/shared/infrastructure/config/LogiBezPiiIT.java`
- [ ] T155 [P] Przegląd dostępności i responsywności (360 px, siatka 2/3/4, fokus klawiatury, `aria-disabled` z wyjaśnieniem, kontrast) i poprawki w `frontend/src/app/Layout.module.css`, `frontend/src/features/katalog/KatalogPage.module.css`, `frontend/src/features/koszyk/KoszykPage.module.css`, `frontend/src/features/zamowienie/ZamowieniePage.module.css`
- [ ] T156 [P] Dodaj `README.md` w katalogu głównym (wymagania, `.env` z `GRAFANA_ADMIN_PASSWORD`, `docker compose up -d`, uruchomienie backendu/frontendu, adresy Grafany `http://127.0.0.1:3000` i Prometheusa `http://127.0.0.1:9090`, testy, gitleaks, `promtool`) z odnośnikiem do `specs/001-shop-browse-cart-checkout/quickstart.md`
- [ ] T157 Uzupełnij job E2E w `.github/workflows/ci.yml` (compose + backend profil `e2e` + `stripe listen` z sekretami CI, uruchomienie `npm --prefix frontend run e2e`, pomijany gdy brak `STRIPE_TEST_*`) i profil `backend/src/main/resources/application-e2e.yaml` (seed jak `local`)
- [ ] T158 Uruchom `gitleaks detect --no-banner` i audyt zależności lokalnie (`osv-scanner scan source --lockfile backend/pom.xml --lockfile frontend/package-lock.json` jak w T010 + `npm --prefix frontend audit --audit-level=high`); usuń znaleziska high/critical
- [ ] T159 Przejdź ręcznie wszystkie scenariusze z `specs/001-shop-browse-cart-checkout/quickstart.md` (sekcje 3 i 4, w tym US5 5.1–5.12) i odnotuj wynik w opisie PR
...
- **US5 (Phase 7)**: po US4 — metryki wpinane są w serwisy US2–US4 (T137 → `KoszykService`, T138 → `ZlozZamowienieService`/`ObslugaPlatnosciListener`, T140 → `ObslugaWebhookaService`, T141 → `StripeBramkaPlatnosci` z T102) i rozszerzają ich testy IT (T127–T129). Wspólne jądro (T120–T121, T134–T136) oraz cała konfiguracja Prometheusa/Grafany (T132–T133, T143–T150) zależą tylko od Phase 2 i kontraktu `contracts/metrics.md` — mogą powstawać równolegle z US1–US4.
- **Polish (Phase 8)**: po wszystkich historiach (T153 mierzy też narzut metryk z US5).

### User Story Dependencies

```text
Phase 1 ─► Phase 2 ─┬─► US1 ─► US2 ─► US3 ─► US4 ─► US5 (metryki w BC) ─► Polish
                    ├─► (równolegle) US4: domena + adapter Stripe (T083–T086, T100–T103)
                    └─► (równolegle) US5: jądro + observability/** (T120–T121, T132–T136, T143–T150)
```
...
- US5: testy T120–T126 i T130–T133 równolegle; T127–T129 równolegle między sobą po T126; jądro T134–T136 równolegle; T142 równolegle z T137–T141; pliki `observability/**` T143–T150 wszystkie równolegle (każdy to osobny plik).
- Polish: T152–T156 równolegle.
...
```bash
# Konfiguracja obserwowalności — niezależna od kodu, od razu po Phase 2:
Task: "Testy promtool w observability/prometheus/tests/shop.test.yml"
Task: "Reguły alertów w observability/prometheus/rules/shop.yml"
Task: "Provisioning Grafany w observability/grafana/provisioning/"
Task: "Dashboard HTTP w observability/grafana/dashboards/http.json"
Task: "Dashboard ścieżki zakupowej w observability/grafana/dashboards/sciezka-zakupowa.json"
Task: "Walidator scripts/check-dashboards.mjs"

# Testy adapterów metryk — po jednym na BC:
Task: "MicrometerMetrykiKoszykaTest w backend/src/test/java/com/project/custom/koszyk/infrastructure/metrics/"
Task: "MicrometerMetrykiZamowienTest w backend/src/test/java/com/project/custom/zamowienie/infrastructure/metrics/"
Task: "MicrometerMetrykiPlatnosciTest w backend/src/test/java/com/project/custom/platnosc/infrastructure/metrics/"
```
...
6. US5 → dashboardy, alerty i metryki bez PII po `docker compose up -d` (SC-009–SC-012).
7. Polish → kontrakt, wydajność i narzut metryk, logi bez PII, przejście quickstart (SC-008).
...
2. Następnie:
   ...
   - Dev C: frontend US4 na MSW według kontraktu OpenAPI, potem US5 `observability/**` (reguły, testy `promtool`, dashboardy) według `contracts/metrics.md`
3. Integracja US4 po zakończeniu US3; metryki w BC (T137–T142) po US4.
